...
# ============ RESOLUÇÃO DE CADA NÍVEL ============
def resolver_nivel(nome_base):
    saida = [f"\n=== RESULTADOS PARA {nome_base.upper()} ==="]
    A = ler_matriz(f"/tmp/{nome_base}_A.txt")
    f = ler_vetor(f"/tmp/{nome_base}_f.txt")
    n = len(f)
    I = identidade(n)
    ...
    for i in range(n):
        linha = f"Setor {i+1:02d} | Demanda: {f[i]:,.2f} | A: {x_A[i]:,.2f} | Jacobi: {x_B[i]:,.2f} | GS: {x_C[i]:,.2f} | SOR: {x_D[i]:,.2f} | Grad: {x_G[i]:,.2f} | CG: {x_CG[i]:,.2f} | Exata: {x_exact[i]:,.2f}"
        saida.append(linha)

    saida += [
        "\n--- RESUMO ---",
        f"Splitting A: Iterações = {it_A}, Tempo = {t_A:.4f} s",
        f"Jacobi (B): Iterações = {it_B}, Tempo = {t_B:.4f} s",
        f"Gauss-Seidel (C): Iterações = {it_C}, Tempo = {t_C:.4f} s",
        f"SOR (D): Iterações = {it_D}, Tempo = {t_D:.4f} s",
        f"Gradiente: Iterações = {it_G}, Tempo = {t_G:.4f} s",
        f"Gradiente Conjugado: Iterações = {it_CG}, Tempo = {t_CG:.4f} s"
    ]
...
def lambda_handler(event, context):
    print("Iniciando execução no Lambda...")
    baixar_arquivos_do_s3()

    saida_geral = []
    for nivel in ["nivel12", "nivel20", "nivel67"]:
        saida_geral.append(resolver_nivel(nivel))

    conteudo = "\n\n".join(saida_geral)
    caminho_arquivo = "/tmp/resultados_lambda.txt"
    
    with open(caminho_arquivo, "w", encoding="utf-8") as f:
        f.write(conteudo)

    s3.upload_file(caminho_arquivo, BUCKET_NAME, "resultados_lambda.txt")
    print("Arquivo resultados_lambda.txt enviado para o S3 com sucesso.")

    return {
        "statusCode": 200,
        "body": "Execução concluída. Resultados salvos no S3."
    }
